...
from .algorithms import (
    Algorithm,
    BioCLIP25LogRegClassifier,
    BioCLIPPanamaLogRegClassifier,
    ConstantClassifier,
    HFImageClassifier,
...
class BioCLIP25LogRegPipeline(Pipeline):
    """
    Zero-shot object detector for bounding boxes, then BioCLIP 2.5 + logistic-regression
    head for the terminal species classification.

    The detector is a placeholder for local testing. Swap stage 0 for the production
    moth detector before using this on real deployments.
    """

    batch_sizes = [1, 1]
    config = PipelineConfigResponse(
        name="BioCLIP 2.5 + LogReg Head Pipeline",
        slug="bioclip-2-5-logreg-pipeline",
        description="Zero shot object detector with a BioCLIP 2.5 + logistic-regression species classifier.",
        version=1,
        algorithms=[
            ZeroShotObjectDetector().algorithm_config_response,
            BioCLIP25LogRegClassifier().algorithm_config_response,
        ],
    )
    ...
    def get_stages(self) -> list[Algorithm]:
        ...
        classifier = BioCLIP25LogRegClassifier()
        self.config.algorithms = [
            zero_shot_object_detector.algorithm_config_response,
            classifier.algorithm_config_response,
        ]
        return [zero_shot_object_detector, classifier]
    ...
    def run(self) -> PipelineResultsResponse:
        ...
        logger.info("[2/2] Running the BioCLIP 2.5 classifier...")
        detections_with_classifications = self._get_detections(self.stages[1], detections, self.batch_sizes[1])

        elapsed_time = (datetime.datetime.now() - start_time).total_seconds()
        pipeline_response = self._get_pipeline_response(detections_with_classifications, elapsed_time)
        logger.info(f"Successfully processed {len(detections_with_classifications)} detections.")
...
@final
class BioCLIPPanamaPipeline(BioCLIP25LogRegPipeline):
    """
    Same two stages as the Newfoundland pipeline, with the Panama head as the terminal
    classifier. Both pipelines share the frozen encoder through SAVED_MODELS, so running
    both on one GPU costs one copy of the backbone plus two small Linear heads.
    """

    batch_sizes = [1, 1]
    config = PipelineConfigResponse(
        name="BioCLIP 2.5 + Panama LogReg Head Pipeline",
        slug="bioclip-2-5-panama-pipeline",
        description="Zero shot object detector with a BioCLIP 2.5 + Panama logistic-regression species classifier.",
        version=1,
        algorithms=[
            ZeroShotObjectDetector().algorithm_config_response,
            BioCLIPPanamaLogRegClassifier().algorithm_config_response,
        ],
    )
    ...
    def get_stages(self) -> list[Algorithm]:
        ...
        classifier = BioCLIPPanamaLogRegClassifier()
        self.config.algorithms = [
            zero_shot_object_detector.algorithm_config_response,
            classifier.algorithm_config_response,
        ]
