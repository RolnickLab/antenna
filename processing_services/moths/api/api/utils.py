...
def get_or_download_file(path_or_url, tempdir_prefix="antenna") -> pathlib.Path:
    ...
    cache_root = pathlib.Path("/app/cache") if pathlib.Path("/app").exists() else pathlib.Path.home() / ".antenna_cache"
...
def open_image(
    fp: str | bytes | pathlib.Path | io.BytesIO, raise_exception: bool = True
