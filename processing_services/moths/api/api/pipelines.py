...
class Pipeline:
    ...
    def __init__(
        ...
        self.request_config = (
            ...
            else request_config.model_dump()
        )
        ...
        self.batch_sizes = (
            custom_batch_sizes or self.batch_sizes or [1] * len(self.stages)
        )
        assert len(self.batch_sizes) == len(
            self.stages
        ), "Number of batch sizes must match the number of stages."
    ...
    def compile(self):
        ...
        for stage_idx, stage in enumerate(self.stages):
            logger.info(
                f"[{stage_idx+1}/{len(self.stages)}] Compiling {stage.algorithm_config_response.name}..."
            )
            stage.compile()
    ...
    def _get_detections(
        self,
        algorithm: Algorithm,
        inputs: list[SourceImage] | list[Detection],
        batch_size: int,
        **kwargs,
    ) -> list[Detection]:
    ...
    def _get_pipeline_response(
        self, detections: list[Detection], elapsed_time: float
    ) -> PipelineResultsResponse:
        """
        ...
        source_image_responses = [
            SourceImageResponse(**image.model_dump()) for image in self.source_images
        ]
...
class ZeroShotHFClassifierPipeline(Pipeline):
    ...
    def get_stages(self) -> list[Algorithm]:
        ...
        if "candidate_labels" in self.request_config:
            logger.info(
                "Setting candidate labels for zero shot object detector to %s",
                self.request_config["candidate_labels"],
            )
            zero_shot_object_detector.candidate_labels = self.request_config[
                "candidate_labels"
            ]
    ...
    def run(self) -> PipelineResultsResponse:
        ...
        logger.info(
            f"Successfully processed {len(detections_with_classifications)} detections."
        )
