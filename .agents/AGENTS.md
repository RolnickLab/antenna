...
- **Changing `VALID_JOB_TYPES` requires a migration.** `Job.job_type_key` builds its `choices` from the registry, so adding/renaming a job type changes the field definition. The migration is state-only (no schema change) but Django requires it — this has been forgotten before (see migration `0023_alter_job_job_type_key`).
