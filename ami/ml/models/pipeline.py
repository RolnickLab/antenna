...
def get_or_create_detection(
    ...
    if detection_resp.crop_image_url and detection_resp.crop_image_url.startswith(("http://", "https://")):
        # Only reuse an externally-hosted crop URL. Relative paths (or None) fall through so a
        # crop is generated locally by the background task instead.
...
def save_results(
    ...
    if create_new_algorithms:
        # The importer path registers algorithms and category maps that arrive with externally
        # produced results. Normally these are created in advance when registering the pipeline &
        # processing service, but they are also available in each pipeline results response.
        # @TODO review if we should only use the algorithms from the pre-registered pipeline config
        ...
        # Add all algorithms reported in the pipeline response to the pipeline
        for algo in algorithms_known.values():
            pipeline.algorithms.add(algo)

        algos_reported = [
            f"    {algo.task_type}: {algo_key} ({algo})\n" for algo_key, algo in algorithms_known.items()
        ]
        job_logger.info(f"Algorithms reported in pipeline response: \n{''.join(algos_reported)}")
    else:
        algorithms_known = {algo.key: algo for algo in pipeline.algorithms.all()}
